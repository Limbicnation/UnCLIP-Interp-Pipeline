...
# Use polynomial interpolation instead of slerp
def linear_interp(val, low, high):
    """
    Find the interpolation point between the 'low' and 'high' values for the given 'val' using linear interpolation.
    """
    res = np.interp(val, [0, 1], np.vstack((low, high)))
    return res
...
class UnCLIPImageInterpolationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        #using Linear Interpolation with interp1d function

        image_embeddings = []
        interp_points = torch.linspace(0, 1, steps)
        interp_func = interp1d([0, 1], torch.cat(original_image_embeddings).numpy(), axis=0)

        for interp_step in interp_points:
            temp_image_embeddings = torch.from_numpy(interp_func(interp_step)).unsqueeze(0)
        ...
        # use CLIP instead of additive time embeddings
        for i, t in enumerate(self.progress_bar(decoder_timesteps_tensor)):
            # expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([decoder_latents] * 2) if do_classifier_free_guidance else decoder_latents

            with torch.no_grad():
                # generate text embeddings for each step
                prompts = [" ".join(
                    ["<{}>".format(self.clip_model.tokenizer.decode([idx])) for idx in self.clip_model.encode(prompt)])
                           for prompt in self.prompts]
                clip_tokens = self.clip_model.encode(prompts).to(device)
                clip_embeddings = self.clip_model.encode_image(image_embeddings, convert_images=True).float().to(device)
                clip_embeddings = clip_embeddings / clip_embeddings.norm(dim=-1, keepdim=True)
                clip_tokens_embed = self.clip_model.embed_tokens(clip_tokens)
                clip_tokens_embed = F.normalize(clip_tokens_embed, dim=-1)

                # calculate logits for each prompt and image embedding
                logits_per_prompt = torch.matmul(clip_tokens_embed, clip_embeddings.t())
                logits_per_prompt /= self.clip_temperature
                clip_probs = F.softmax(logits_per_prompt, dim=-1)

                # get the weighted sum of image embeddings
                class_labels = torch.matmul(clip_probs, clip_embeddings)
        ...
        # interpolate
        def interpolate_image(image_small, height, width, device, interpolation_factor=0.65):
            if device.type == "mps":
                ...
                # Adjust the strength of the interpolation
                if interpolation_factor < 1.0:
                    image_upscaled = F.interpolate(
                        image_small, scale_factor=interpolation_factor, mode="bilinear", align_corners=False,
                        **interpolate_antialias
                    )
                    image_upscaled = F.interpolate(image_upscaled, size=[height, width], mode="bicubic",
                                                   align_corners=False)
                else:
                    image_upscaled = F.interpolate(
